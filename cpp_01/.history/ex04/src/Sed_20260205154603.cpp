...
/*   Updated: 2026/02/05 15:46:03 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Sed::run() const
{
	...
	while (std::getline(infile, line))	// read file line by line
	{
		...
		while ((pos = line.find(s1, last)) != std::string::npos)
		{
			...
			result += s2;				// append the replacement string s2
			last = pos + s1.length();
		}
		result += line.substr(last);	//	append whats leftof the line after the last occurrence of s1
