...
/*   Updated: 2026/02/05 14:40:14 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Sed::run() const
{
	...
	if (s1.empty())
	{
		...
		return;  // exit early if nothing to search
	}
	...
	// Open/create the output file "<filename>.replace" for writing
	std::ofstream outfile((filename + ".replace").c_str());
	if (!outfile)  // check if file creation was successful
	{
		std::cerr << "Error: cannot create output file.\n";
		return;
	}

	std::string line;  // variable to hold each line read from input file
